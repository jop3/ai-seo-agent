...
from src.azure_agents.definitions import AgentDefinition, ToolDefinition, ALL_TOOLS
...
class LocalAgentRunner(AgentRunner):
    ...
    async def run_tool(
        ...
        """
        Execute a tool by calling the local API endpoint.
        """
        if not tool.api_endpoint:
            raise ValueError(f"Tool {tool.name} has no API endpoint defined")
