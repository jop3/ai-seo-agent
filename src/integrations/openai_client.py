...
- Local models via OpenAI-compatible endpoints (Docker Model Runner, Ollama, etc.)
"""
...
from src.config import AzureSettings, Settings
...
class OpenAIClient:
    ...
    def __init__(self, settings: AzureSettings | Settings):
        self.settings = settings
        self._client = None

    def _get_client(self) -> AsyncAzureOpenAI | AsyncOpenAI:
        """Lazy initialization of Azure OpenAI or standard OpenAI client."""
        if self._client is None:
            # Check if using standard OpenAI API (e.g., Docker Model Runner)
            if isinstance(self.settings, Settings) and self.settings.openai_api_base:
                self._client = AsyncOpenAI(
                    api_key=self.settings.openai_api_key or "not-needed",
                    base_url=self.settings.openai_api_base,
                )
            elif hasattr(self.settings, 'openai_api_base') and getattr(self.settings, 'openai_api_base', None):
                # Settings object with openai_api_base
                self._client = AsyncOpenAI(
                    api_key=getattr(self.settings, 'openai_api_key', 'not-needed'),
                    base_url=self.settings.openai_api_base,
                )
            else:
                # Azure OpenAI
                azure_settings = self.settings if isinstance(self.settings, AzureSettings) else self.settings.azure
                self._client = AsyncAzureOpenAI(
                    api_key=azure_settings.openai_api_key.get_secret_value(),
    ...
    async def chat(
        ...
        if isinstance(self.settings, Settings) and self.settings.openai_model:
            model = self.settings.openai_model
        elif hasattr(self.settings, 'openai_model'):
            model = getattr(self.settings, 'openai_model', 'gpt-3.5-turbo')
        elif isinstance(self.settings, AzureSettings):
            model = self.settings.openai_deployment
        else:
            model = self.settings.azure.openai_deployment
...
# Backwards compatibility alias (deprecated, use OpenAIClient instead)
